...
namespace lve
{
	...
	FirstApp::FirstApp()
	{
		globalPool =
			...
			.setMaxSets(LveEngineSwapChain::MAX_FRAMES_IN_FLIGHT)
			.addPoolSize(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, LveEngineSwapChain::MAX_FRAMES_IN_FLIGHT)
			.build();
	...
	void FirstApp::run()
	{
		std::vector<std::unique_ptr<LveBuffer>> uboBuffers(LveEngineSwapChain::MAX_FRAMES_IN_FLIGHT);
		...
		std::vector<VkDescriptorSet> globalDescriptorSets(LveEngineSwapChain::MAX_FRAMES_IN_FLIGHT);
